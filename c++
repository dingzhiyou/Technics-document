...
# 左值右值
一般来说，能取变量得值就是左值，否则就是右值（典型的就是临时变量）。
# 左值引用，又值引用
接收左值的引用就是左值引用符号为&，接收右值的引用就是右值引用符号为&&，右值引用主要是延长临时变量的生命周期
std::move就是将一个对象转换为右值引用，从而告诉编译器可以将其移动到另一个对象而不是拷贝（在提供移动构造函数的情况下）
std::forward()通常与模版和引用折叠相关的完美转发一起使用，它允许在函数模版中完美转发参数，保持参数的值分类（左值，右值）不变。
template<class T> 
void process(T&& arg) //万能模版，既能接受左值，也能接受右值，这是编译器自己确定了规则
{
