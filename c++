...
#永远不要在构造函数中调用虚函数：构造的过程是先构造基类，在构造子类，如果子类重写了基类的虚函数，那么在构造基类的过程中调用了虚函数的话是调用了基类的虚函数，因为这时候子类的数据还没初始化好
那么解决的办法是什么呢，就是在基类中不调用虚函数，而是通过子类将数据传送给基类去实现(初始化参数列表时可以显示传参给基类的构造函数)，然后在基类构造函数中进行操作。始终要牢记一点，首先构造基类，此时子类还没有开始构造，不能去拿子类的函数或成员
实际上一个类的构造过程是先构造基类，在构造成员变量，在调用构造函数。既然基类无法通过虚函数拿到子类的数据(向下调用不允许)，那我们就可以向上传递(子类将数据传递给基类构造函数)

#重载赋值运算符记得返回自身的引用，重载的时候特别注意判断一下是不是自身给自身赋值，因为这样可能在赋值函数对堆上的数据进行操作，导致错误

#继承的情况下，子类在拷贝构造和赋值运算的时候也有很多隐藏的问题，比如说我拷贝构造的时候你不显示调用基类的构造函数，就会调用默认的构造函数，否则会报错找不到默认构造函数
如果采用了默认构造，没有使用专门的构造函数，会不会出现拷贝的基类数据不一致。同样在进行赋值运算的时候也要为基类进行赋值，调用基类的operator=函数。

#RAII对象时该如何处理，大多数时候你会有如下选择:
1.禁止复制
2.引用计数底层资源
